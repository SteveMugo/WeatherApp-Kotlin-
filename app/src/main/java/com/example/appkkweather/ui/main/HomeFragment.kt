...
import com.example.appkkweather.databinding.FragmentHomeBinding
import com.example.appkkweather.databinding.FragmentHomeBinding.*
import com.example.appkkweather.model.citiesModel
import com.example.appkkweather.ui.citiesWeather.*
import com.example.appkkweather.ui.currentWeather.CurrentWeather
import com.example.appkkweather.ui.currentWeather.CurrentWeatherViewModel
import com.example.appkkweather.ui.favorite.FavoriteViewModel
import com.example.appkkweather.utils.*
...
class HomeFragment : Fragment(R.layout.fragment_home), cityDetailInterface {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currWViewModel.WeatherService.observe(this, androidx.lifecycle.Observer {
            ...
            //set data
            binding.homeCity.text = currWViewModel.city

            Glide.with(this).load(IconManager().getIcon(currWViewModel.icon))
                .into(binding.homeWeatherIcon)

            binding.homeStatus.text = currWViewModel.description

            binding.homeTemperature.text = currWViewModel.temperature

            binding.homeWaterDrop.text = currWViewModel.water_drop

            binding.homeWindSpeed.text = currWViewModel.wind_speed

            binding.homeFragmentBG.setBackgroundResource(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        context?.let {
            CurrentWeather(
                currWViewModel, binding.currentLayout, binding.homeProgressBar
            ).showCurrentLocationData(it, latitude, longitude)
        }
...
//            val header_binding = navigation.inflateHeaderView(R.layout.header_layout);
//            val header_image = header_binding.setImageResource(IconManager().getIcon(currWViewModel.icon))
//            navigation.inflateHeaderView(R.layout.header_layout).context.(R.id.header_image).set
//            val header_binding = navigation.getHeaderView(R.layout.header_layout)
//            val header_binding = navigation.inflateHeaderView(R.layout.header_layout)
//            val header_image = header_binding.requireViewById<R.id.header_image>
//            header_image.findViewById(R.id.header_image).setImageResource(IconManager().getIcon(currWViewModel.icon))
//            header_binding.findViewById<T>(R.id.header_text) = currWViewModel.description
//            navigation.header_bg.setBackgroundResource(BackgroundManager().getBackground(currWViewModel.description))
        ...
        //on city clicked
        binding.homeCity.setOnClickListener {
        ...
        val user = context?.let { Firebase.auth.currentUser }
    ...
    private fun showCurrentLocationForecast() {
        onItemClick(
            ...
            currWViewModel.icon,
            currWViewModel.description,
            currWViewModel.temperature,
            currWViewModel.wind_speed,
            currWViewModel.water_drop,
            currWViewModel.min_temp,
            currWViewModel.max_temp,
            currWViewModel.lat,
            currWViewModel.long
        )
    }
    ...
    private fun getCitiesWeather() {
        ...
        viewModel.liveData.observe(viewLifecycleOwner, androidx.lifecycle.Observer {
            ...
            *       {"temp":288.99,
                        "feels_like":288.56,
                        "temp_min":288.99,
                        "temp_max":288.99,"pressure":1024,"humidity":74,"sea_level":1024,"grnd_level":768},"visibility":10000,"wind":{"speed":1.97,"deg":5,"gust":1.85},"clouds":{"all":93},"dt":1753710791,"sys":{"country":"MX","sunrise":1753704685,"sunset":1753751661},"timezone":-21600,"id":3530597,"name":"Mexico City","cod":200}
            * */
                ...
                if (document != null) { //check if document is empty
                    ...
                    if (cities != null) {
                        for (cityName in cities.values) {
                            Log.d("Cities", cityName.toString())
    ...
    override fun onItemClick(
        ...
        (activity as MainActivity?)?.showDetailedWeather(
            detailedFragment, bundle, binding.homeTemperature
        )
    ...
    private fun initSearch() {
        ...
        binding.searchBtn.setOnClickListener {
            ...
            binding.currentLayout.startAnimation(
            ...
            binding.RecyclerViewLayout.startAnimation(
                AnimationUtils.loadAnimation(
    ...
    //init filter search
    fun filter(
        city: String, data: ArrayList<citiesModel>, recyclerView: RecyclerView, adapter: CityAdapter
    ) {
